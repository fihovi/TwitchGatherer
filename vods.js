...
global.sql = 'INSERT IGNORE INTO F_Videa.vods (id, user_id, user_name, title, description, created_at, published_at, url, thumbnail_url, viewable, view_count, language, type, duration, stream_id) VALUES ?';
global.sqlUpdate = '';
...
function checkUser(user, uid, dname){ //Username, User_id, Display Name
	...
	if (response.user === true && response.uid === true && response.dname === true){
		...
		//MySQL Query UPDATE F_Videa.vods SET streamer=xxx WHERE 
	}
